...
不预建任何其他文件或目录。TASKS.md 在第一个任务出现时创建;`tasks/<id>/` 在第二个并发任务出现时才分裂;`dev-log/` 在第一次落证据时创建;`wiki/` 在第一个词条入库时创建;`CONVENTIONS.md` 在第一条长期协作规范(职责边界)确认时创建;`spec/` 在第一份本项目技术规范出现时创建。

### 知识归位判定(写任何一条知识前先过这棵树,放错地方=污染)
- 是资源坐标(机器/host/密钥位置/怎么访问)?→ **HANDOFF**;
- 是"我们怎么协作"(谁执行、评审规则、工作流)?→ **CONVENTIONS.md**;
- 是**本项目专属的技术/设计规范**(设计语言、代码约定、词汇表、架构决策、只对本项目成立的规则与踩坑)?→ **`spec/<主题>.md`**;有状态、会演进、只对本项目有用,**永不进 wiki、永不同步中央库**;
- 是**无状态、跨项目通用**、换个完全无关的项目也有用的问题解答?→ **`wiki/`**(唯一同步中央 llm-wiki 的层)。
- **一句话判据**:「换一个毫不相干的项目,这条还有用吗?」有 → wiki;只对本项目有用 → spec。项目已有的 LESSONS/experience 类文件保留原位,通用的在 wiki index 挂链、专属的归 spec。
