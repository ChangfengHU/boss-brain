...
install_cmd_skills() { # $1=skills dir; 四命令各发布为独立 skill(紧凑显示,可被模型按需触发)
  ...
  for c in "$SRC/commands/"*.md; do
    ...
    # A standalone skill must ship every guide it tells the model to read — the installed
    # topology has to match the skill text, or the instruction fails exactly when followed.
    local g
    for g in $(grep -o 'guides/[a-z-]*\.md' "$1/$n/SKILL.md" 2>/dev/null | sort -u); do
      [ -f "$SRC/skill/project-brains/$g" ] || continue
      mkdir -p "$1/$n/guides"
      cp "$SRC/skill/project-brains/$g" "$1/$n/$g"
    done
  done
}
